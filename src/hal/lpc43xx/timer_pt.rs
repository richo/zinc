...
fn build_timer(builder: &mut Builder, cx: &mut ExtCtxt, node: Rc<node::Node>) {
  ...
  let timer_name = match timer_index {
    0...3 => TokenString(format!(
        "zinc::hal::lpc43xx::timer::TimerPeripheral::Timer{}", timer_index)),
    other => {
  ...
  node.set_type_name("zinc::hal::lpc43xx::timer::Timer".to_string());

  let st = quote_stmt!(&*cx,
      let $name = zinc::hal::lpc43xx::timer::Timer::new(
  ...
  fn builds_timer() {
    with_parsed("
      ...
      assert_equal_source(&builder.main_stmts()[0],
          "let tim = zinc::hal::lpc43xx::timer::Timer::new(
